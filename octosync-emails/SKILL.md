...
`company {name, domain, website, phone}`,
`contact {email, name, role, isGenericInbox}`, `decision`, `decidedAt`.
The business phone lives on the **Company**, the person on the
**Contact**. See the script header for the full schema.
