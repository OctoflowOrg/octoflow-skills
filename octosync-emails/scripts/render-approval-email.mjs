// OctoSync LinkedIn-review email renderer + sender.
//
// Used only by linkedin-finalize-batch.mjs. The prospecting flow
// moved into the broker (services/approval-broker/) in #45; this
// file's prospecting functions (renderProspectingApprovalEmail,
// sendProspectingApprovalEmail) were removed in the same PR series
// because the CSO agent kept calling them directly to bypass the
// broker (OCT-363, OCT-367 — both rendered emails with the legacy
// /decide form action). The broker has its own copy of the
// prospecting renderer at services/approval-broker/email-render.mjs;
// see that file as the source-of-truth going forward.
//
