---
name: enercity-coordination-rules
description: >
  ALWAYS load at the start of every heartbeat. Defines the EnerCity Collaborative
  ...
  reading for every EnerCity Collaborative workflow agent before any other action.
  Load this even when the task seems unrelated to coordination — most
  agent failures are coordination failures.
---

# EnerCity Collaborative coordination rules

These rules apply to every EnerCity Collaborative Paperclip workflow agent. Load
...
- **Parent issue** — owned by the orchestrator (the CMA).
...
2. Worker posts the canonical handoff comment on its **own child**
   ...
   full, self-contained handoff content. Do NOT prefix with `@CMA`.
3. Worker sets the final disposition on its own child via
   ...
   `{ "status": "blocked" }` with the named-owner `@CMA Blocker:`
...
- Never leave unresolved shell placeholders such as `$CHILD_ID` in a
  user-facing comment.
- Never @-mention yourself: an orchestrator that posts `@CMA
  Researcher started` self-wakes. CMA status notes on the parent
  (e.g. `Researcher started`, `Drafter started`, `LinkedIn review email
  sent`) MUST omit the self-mention.
...
## EnerCity Collaborative identity

The current Paperclip company and parent issue are authoritative for
what `EnerCity Collaborative` means in this run. For positioning, ICP, industries
in/out of scope, and geographic focus, load the `enercity-collaborative-profile`
...
recovery owner (in EnerCity Collaborative, the CEO by fallback). Failure to honor
...
- **Orchestrator** (the CMA) exits with:
  ...
  - `in_review` when a human approval / review path is in flight
    (the approval-broker patches this transition for the LinkedIn
    approval flow).
...
- **If `PAPERCLIP_WAKE_REASON` is `issue_commented` AND the latest
  ...
  their prefix: `LinkedIn review email sent`. The broker has
...
- Posting a "workflow complete" / "all done" summary.
- Posting an updated approval record even if the wording differs from a
  previous one (e.g., re-posting the approved-option comment after the
  parent is already `done`).
- Re-running idempotent verification steps (re-listing approvals,
  re-reading worker handoffs) just to confirm the terminal state.
...
posted; there is no second summary to post.
...
- `references/bundled-scripts.md` — invocation pattern for in-skill
  scripts (relevant when the agent invokes `send-approval.mjs` from the
  `enercity-emails` skill).
